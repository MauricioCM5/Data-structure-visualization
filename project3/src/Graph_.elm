...
isEulerian nodes edges =
    let
        isConnected = isGraphConnected nodes edges
        allEvenDegree = Dict.values nodes
            |> List.all (\node -> 
...
isGraphConnected nodes edges =
    case Dict.values nodes of
        ...
        first :: _ ->
            let
                visited = dfs first.id nodes edges Dict.empty
            in
...
dfs nodeId nodes edges visited =
    case Dict.get nodeId nodes of
        ...
        Just node ->
            let
                newVisited = Dict.insert nodeId True visited
                neighbors = List.filter (\e -> e.from == nodeId || e.to == nodeId) edges
                    |> List.map (\e -> if e.from == nodeId then e.to else e.from)
...
eulerianCycleHelper current edges path =
    case List.filter (\e -> e.from == current || e.to == current) edges of
        ...
        edge :: restEdges ->
            let
                next = if edge.from == current then edge.to else edge.from
                newEdges = List.filter (\e -> e /= edge) edges
            in
